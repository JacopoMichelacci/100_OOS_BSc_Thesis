...
In the experiments, a fixed position size is used. This keeps the comparison focused on the optimization procedure and the trading rule, rather than on the effects of dynamic position sizing or compounding. The framework also supports fixed-fractional sizing, but this is not the main sizing method used for the thesis results.

Both strategies can attach a hard stop loss to newly opened positions. The stop can be defined either as a percentage move from the entry price or as a fixed notional price distance. Stop-loss execution is handled by the backtester at the open-lot level.
...
When the strategy is already positioned in the opposite direction, the generated order first closes the existing exposure and then opens the target position in the new direction. When stacking is enabled, repeated signals in the same direction can add new independent lots.

The main parameters of this strategy are the fast moving-average length, the slow moving-average length, and the stop-loss value. The fast length must be smaller than the slow length.
...
where \(\sigma_t\) is the rolling standard deviation of recent price moves.
