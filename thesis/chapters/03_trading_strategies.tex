...
or compounding. Although fixed-fractional position sizing could represent the effect of the two
optimization methods more realistically, it is not the sizing method used for the thesis results. The
framework supports dynamic sizing based on current equity values, but it is left outside the empirical
comparison.
...
The moving-average crossover strategy is a trend-following rule. Moving-average rules are a standard
example of technical trading rules in empirical finance. \citet{brock1992} test moving-average and
trading-range-break rules on the Dow Jones Industrial Average and show that such simple rules have
been widely used as empirical tests of market predictability. The strategy used here compares a fast
moving average with a slow moving average. The fast average reacts more quickly to recent price
changes, while the slow average represents a smoother estimate of the price trend.
...
where \(\sigma_t\) is the rolling standard deviation over the selected lookback window.
