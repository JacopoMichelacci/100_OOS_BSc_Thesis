...
Parameter selection is based on robust grid regions rather than isolated metric peaks, because selecting
the single best point after testing many alternatives is exactly the type of data-snooping problem
discussed by \citet{white2000}.
...
resting values (\(\texttt{fast\_len} = 50\), \(\texttt{slow\_len} = 100\)). Figure~\ref{fig:mac-slpct} compares the
...
The stop-loss was left disabled (\(\texttt{slpct} = -1\)) for both the benchmark and the synthetic configuration
...
\(\texttt{lower\_std\_thresh} = -1.5\), \(\texttt{upper\_std\_thresh} = 1.5\)). Figure~\ref{fig:mrev-slpct} sweeps the
...
early profitable region, giving \(\texttt{slpct} = 2.5\%\). This is a risk-control choice, not a pure
...
decays toward the same inactive-stop plateau. The best single point, \(\texttt{slpct} = 0.1\%\), is too isolated
...
\begin{table}[H]
    ...
    \begin{tabular}{lllll}
        \toprule
        Strategy & Method & Parameters & \texttt{slpct} & Other \\
        \midrule
